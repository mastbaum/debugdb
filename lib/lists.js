        ...
        database.getView('tags_with_status', {group_level: 1}, function(err, data) {
            for (var row in data.rows) {
                ...
                if (all_boards.indexOf(board) >= 0) {
                    status = data.rows[row].value[0] ? data.rows[row].value[0] : 'none';
                    var tag = {
            ...
            for (var row in data.rows) {
                ...
                if (all_boards.indexOf(board) >= 0) {
                    ...
                    if (board.substr(0,1) == 'd') {
                        var html = '<br/>';
                        for (var i=0; i<8; i++) {
                            html += '<div style="margin-right:1px;display:inline-block;height:4px;width:4px;background:' + (data.rows[row].value[2][i].good ? 'green' : 'red') + ';"></div>';
                        }
