    ...
    for(var i=0; i<d.QHL_even.length; i++) {
        ...
        for(var j=0; j<d.QHL_even[i].length; j++) {
            t = {
                channel: i,
                val: j,
                qhl_even: d.QHL_even[i][j],
                qhl_odd: d.QHL_odd[i][j],
                qhs_even: d.QHS_even[i][j],
                qhs_odd: d.QHS_odd[i][j],
                qlx_even: d.QLX_even[i][j],
                qlx_odd: d.QLX_odd[i][j],
                tac_even: d.TAC_even[i][j],
                tac_odd: d.TAC_odd[i][j],
    ...
    for(var i=0; i<d.num.length; i++) {
        ...
        for(var j=0; j<d.num[i].length; j++) {
            table.push({
                ...
                num: d.num[i][j],
                qhl: d.qhl[i][j],
                qhl_rms: d.qhl_rms[i][j],
                qhs: d.qhs[i][j],
                qhs_rms: d.qhs_rms[i][j],
                qlx: d.qlx[i][j],
                qlx_rms: d.qlx_rms[i][j],
                tac: d.tac[i][j],
                tac_rms: d.tac_rms[i][j],
            });
        }
        tables.push(table);
    }
