...
exports.tests_by_fec = {
    map: function(doc) {
        if(doc.config) {
            ...
            emit([doc.config.fec_id, created], {'type': doc.type, 'pass': doc.pass, 'created': doc.created, 'loc': doc.config.loc});
        }
    }
};
...
exports.tests_by_crate = {
    map: function(doc) {
        if(doc.config) {
            var c = new Date(doc.created);
            var created = -c.getTime();
            emit([doc.config.crate_id, created], {'type': doc.type, 'pass': doc.pass, 'created': doc.created, 'loc': doc.config.loc});
        }
    }
};
...
exports.tests_by_slot = {
    map: function(doc) {
        if(doc.config) {
            var c = new Date(doc.created);
            var created = -c.getTime();
            emit([{"crate": doc.config.crate_id, "slot": doc.config.slot}, created], {'type': doc.type, 'pass': doc.pass, 'created': doc.created, 'loc': doc.config.loc});
        }
    }
};
        ...
        if(doc.config) {
            doc.config.db.forEach(
               function(db) {
                   var c = new Date(doc.created);
                   var created = -c.getTime();
                   emit([db.db_id, created], {'type': doc.type, 'pass': doc.pass, 'created': doc.created, 'loc': doc.config.loc});
               }
            );
        }
    }
...
exports.tests_by_name = {
    map: function(doc) {
        if(doc.config) {
            var c = new Date(doc.created);
            var created = -c.getTime();
            emit([doc.type, created], {'type': doc.type, 'pass': doc.pass, 'created': doc.created, 'config': doc.config, 'loc': doc.config.loc});
        }
    }
};
